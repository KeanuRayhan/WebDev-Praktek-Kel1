...
const EditMovieForm = ({ movieId }) => {
    ...
    MovieDataService.get(movieId)
      .then(response => {
        ...
        setFormData({
          ...
          year: movieData.year || '',
          country_id: movieData.country_id || '',
          synopsis: movieData.synopsis || '',
          platforms: movieData.Platforms.map((platform) => platform.platform_id),
          genres: movieData.Genres.map((genre) => genre.genre_id),
          actors: movieData.Actors.map((actor) => actor.actor_id),
          link_trailer: movieData.link_trailer || '',
          awardIds: movieData.awardIds || '',
          url_photo: movieData.url_photo || null
        });
      })
      .catch(error => {
  ...
  const handleCheckboxChange = (itemId, field) => {
    setFormData((prevData) => {
      let updatedItems;
      if (prevData[field].includes(itemId)) {
        // Jika item sudah ada, maka hapus item tersebut
        updatedItems = prevData[field].filter(i => i !== itemId);
      } else {
        // Jika item belum ada, maka tambahkan item tersebut
        updatedItems = [...prevData[field], itemId];
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await MovieDataService.updateMovie(movieId, movieData);
  ...
  return (
    <form onSubmit={handleSubmit} className="flex-1 p-8 min-h-screen w-full">
      <div>
        <h1 className="text-2xl font-bold mb-4 text-white">Edit Movie</h1>
      </div>
